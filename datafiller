...
# -*- coding: utf-8 -*-
#
# $Id: datafiller.py 832 2015-11-01 21:55:41Z fabien $
#
# TODO:
#
...
VERSION = '2.0.1-dev'
Id='$Id: datafiller.py 832 2015-11-01 21:55:41Z fabien $'
...
# - 1/2 == 0 (v2) vs 0.5 (v3), int casts become necessary
...
# - PEP 3099 is a summary of bad decisions, or bad arguments for possibly
#   good decisions. an exemple of this is to stick to a LL(1) parser.
#   For the user of a language, this is a non argument, if a LL(2) would
#   allow better (more readable, simpler, ...) syntax.
# - there is a frozenset but no frozenlist.
# - print_function in python2 does not support flush
# - some dict methods have disappeared, i.e. itervalues()
#   2/3: viewvalues() -> values(), values -> list(values())

# some Python pitfalls/horrors:
# - white spaces are significant...
# - some internal types support .clone(), others not, for no clear reason.
# - modifying methods on mutable types do not return "self", so they cannot
#   be combined.
# - @-annotations are semantically significants, it should be in the syntax
# - Python "threads" are a bad useless joke, do not do parallelism with Python
# - integers are mp, that is good until you really need intNN_t for some stuff
# - bad unreadable syntax is called "pythonic", eg:
#   . no do/while
#   . no combined assign/test, use while True + break, assign + if...
#   . exception are used too often where it should not be needed
# - no user-documentation support (as perlpod), just a dev-doc support
# - several dev-doc styles...
# - str/repr recursive consistency is unclear.
#
...
if not opts.self_test_hack and opts.target != 'csv':
    ...
    print("-- Get latest version from http://www.coelho.net/datafiller.html")
    print('')
